...
describe UrlGroup do
  describe :relationships do
    ...
    it { should have_many(:guidance_urls).dependent(:restrict) }
    it { should have_many(:series_urls).dependent(:restrict) }
  ...
  describe 'content_type' do
    it 'should return the content type of the first for_scraping url' do
      ...
      url_group.guidance_urls << build(:url, content_type: content_type, for_scraping: true)
  ...
  describe 'scrape_finished' do
    before :each do
      @url_group = create :url_group
      2.times { @url_group.guidance_urls << create(:url, scrape_finished: true, for_scraping: true) }
      @url_group.guidance_urls << create(:url, scrape_finished: false, for_scraping: false)
    end
    ...
    it 'should be false if there is one or more scrapable urls marked as not scrape_finished' do
      @url_group.guidance_urls << create(:url, scrape_finished: false, for_scraping: true)
